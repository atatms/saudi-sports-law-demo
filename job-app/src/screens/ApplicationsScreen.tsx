...
const STATUS_META: Record<ApplicationStatus, { label: string; bg: string; fg: string; stageIndex: number }> = {
  applied: { label: 'تم التقديم', bg: colors.chipBg, fg: colors.textMuted, stageIndex: 0 },
  review: { label: 'مراجعة', bg: colors.goldSoft, fg: colors.gold, stageIndex: 1 },
  interview: { label: 'مقابلة', bg: colors.primaryLight, fg: colors.primary, stageIndex: 2 },
  offer: { label: 'عرض', bg: colors.successBg, fg: colors.primary, stageIndex: 3 },
  rejected: { label: 'لم يتم الاختيار', bg: colors.dangerBg, fg: colors.danger, stageIndex: 1 },
};
    ...
    <Card style={styles.appCard}>
      <View style={styles.appHeader}>
        ...
        <View style={{ flex: 1, marginHorizontal: spacing.md }}>
          ...
          <Text style={styles.appCompany}>{app.company} · {app.city}</Text>
        </View>
      ...
      {app.note ? (
        ...
        <Text style={styles.dateText}>{app.dateLabel}</Text>
      )}
      ...
      <View style={styles.appActions}>
        {app.status === 'interview' && (
          <TouchableOpacity style={styles.primaryAction} activeOpacity={0.85}>
            ...
            <Text style={styles.primaryActionText}>الاستعداد بالذكاء الاصطناعي</Text>
          </TouchableOpacity>
        )}
        {app.status === 'offer' && (
          <>
            <TouchableOpacity style={styles.ghostAction} activeOpacity={0.85}>
              <Text style={styles.ghostActionText}>تفاوض</Text>
            </TouchableOpacity>
            <TouchableOpacity style={styles.primaryAction} activeOpacity={0.85}>
        ...
        <TouchableOpacity style={styles.ghostAction} activeOpacity={0.85} onPress={onView}>
          <Text style={styles.ghostActionText}>عرض</Text>
        </TouchableOpacity>
      </View>
    </Card>
  );
    ...
    <SafeAreaView style={styles.safe} edges={['top']}>
      <View style={styles.header}>
        <View style={styles.headerRow}>
          ...
          <Text style={styles.total}>{applicationStats.total} إجمالي</Text>
        </View>
